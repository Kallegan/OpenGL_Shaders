...
ShaderProgramSource Shader::ParseShader(const std::string& filepath)
{
    ...
    {
        if (line.find("#shader") != std::string::npos) //doesnt contains invalid stringpos.
        {
...
{
    GLCall(glUniform1i(GetUnitformLocation(name), value));
}

void Shader::SetUniform1f(const std::string& name, float value)
{
    GLCall(glUniform1f(GetUnitformLocation(name), value));
}


void Shader::SetUniform4f(const std::string& name, float v0, float v1, float v2, float v3)
{
    GLCall(glUniform4f(GetUnitformLocation(name), v0, v1, v2, v3));
}
